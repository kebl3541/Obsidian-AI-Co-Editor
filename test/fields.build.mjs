...
// src/comments.ts
var COMMENT_RE = /%%\s*([A-Za-z0-9_-]+)\s*:\s*([\s\S]*?)%%/g;

// src/marks.ts
var SLOT_COUNT = 6;
var addExternalMarks = StateEffect.define();
var clearExternalMarks = StateEffect.define();
var removeMarkRange = StateEffect.define();
var slotMarks = [];
  ...
  update(deco, tr) {
    ...
    for (const e of tr.effects) {
      if (e.is(addExternalMarks)) {
        ...
        deco = deco.update({
          add: e.value.ranges.map((r) => ({ from: Math.min(r.from, max), to: Math.min(r.to, max) })).filter((r) => r.to > r.from).map((r) => mark.range(r.from, r.to)),
          sort: true
        });
...
// src/inline.ts
var setInlineProposals = StateEffect.define();
var clearInlineProposals = StateEffect.define();
var AddWidget = class extends WidgetType {
  constructor(text, proposalIndex, path) {
    super();
    this.text = text;
  ...
  eq(other) {
    return other.text === this.text && other.proposalIndex === this.proposalIndex && other.path === this.path;
  }
  toDOM(view) {
    const doc2 = view.dom.ownerDocument;
...
var delMark = Decoration.mark({
  class: "live-coedit-prop-del",
  attributes: { title: "Proposed deletion" }
});
var inlineProposalsField = StateField.define({
  create: () => Decoration.none,
  update(deco, tr) {
    try {
      deco = deco.map(tr.changes);
    ...
    for (const e of tr.effects) {
      if (e.is(setInlineProposals)) {
        ...
        for (const d of spec.dels) {
          ...
          if (to > from)
            ranges.push(delMark.range(from, to));
        }
